    ...
    <div class="sync-card" style="text-align:left">
      ...
      el.innerHTML = authCard + remindersCard + `
    <div class="sync-card">
      <h3>Export for Claude</h3>
      <p>Download your tasks and daily log so Claude Cowork can read your progress from OneDrive.</p>
      <button class="sync-btn primary" onclick="downloadTasksMd()">Download TASKS.md</button>
      <button class="sync-btn secondary" onclick="downloadDailyLog()">Download daily-log.md</button>
      <button class="sync-btn tertiary" onclick="downloadBoth()">Download Both</button>
      <div class="sync-status" id="syncStatus">Saved! Replace the files in your OneDrive folder.</div>
      <p style="margin-top:12px;font-size:12px">Last file export: ${lastSync}</p>
    </div>
    <div class="sync-card">
      <h3>Copy for Claude</h3>
      <p>Copy today's progress summary to paste into Claude mobile app for quick advice.</p>
      <button class="sync-btn tertiary" onclick="copyForClaude()">Copy Today's Summary</button>
      <div class="sync-status" id="copyStatus">Copied to clipboard!</div>
    </div>
    <div class="sync-card">
      <h3>Auto-Sync from Claude</h3>
      <p>Connect your "Ladson with Malveon" folder once. The app will check for new tasks from Claude every 5 minutes automatically.</p>
      ${workspaceDirHandle
          ? `<div style="display:flex;gap:8px;flex-direction:column">
            <button class="sync-btn primary" onclick="manualSyncFromWorkspace()">Sync Now</button>
            <button class="sync-btn tertiary" onclick="disconnectWorkspace()" style="color:var(--red)">Disconnect Folder</button>
           </div>
           <div class="sync-status" id="workspaceStatus" style="display:none">Synced!</div>
           <p style="margin-top:8px;font-size:12px;color:var(--green)">Folder connected. Checks every 5 minutes.</p>`
          : `<button class="sync-btn secondary" onclick="connectWorkspaceFolder()">Connect Workspace Folder</button>
           <div class="sync-status" id="workspaceStatus" style="display:none">Connected!</div>
           <p style="margin-top:8px;font-size:12px;color:var(--text-dim)">Works on Chrome/Edge desktop. On mobile, use Import below.</p>`
        }
    </div>
    <div class="sync-card">
      <h3>Remove Duplicates</h3>
      <p>Finds tasks with the same name and removes the extra copies. Keeps completed tasks over pending ones.</p>
      <button class="sync-btn tertiary" onclick="removeDuplicateTasks()" style="color:var(--red)">Remove Duplicates</button>
      <div class="sync-status" id="dedupeStatus" style="display:none">Done!</div>
    </div>
    <div class="sync-card">
      <h3>Import from Claude (Manual)</h3>
      <p>Pick the TASKS.md file manually to import tasks Claude has added. Use this on mobile.</p>
      <input type="file" id="importFileInput" accept=".md" style="display:none" onchange="handleImportFile(event)">
      <button class="sync-btn tertiary" onclick="document.getElementById('importFileInput').click()">Import from TASKS.md</button>
      <div class="sync-status" id="importStatus">Tasks imported!</div>
    </div>
    <div class="sync-card" style="text-align:left">
      <h3>How Sync Works</h3>
      <p style="text-align:left;line-height:1.6">
        ${isSignedIn ? '<strong style="color:var(--green)">Device sync:</strong> Your tasks sync automatically between devices via Supabase.<br><br>' : ''}
        <strong style="color:var(--accent)">Claude sync:</strong><br>
        1. Complete your tasks and write your daily review<br>
        2. Tap "Download Both" above<br>
        3. Save the files to your OneDrive "Ladson with Malveon" folder<br>
        4. Claude Cowork reads these files and adjusts your tasks
      </p>
    </div>`;
    }
  ...
  `).join('');
    ...
    <div class="sync-card">
      ...
      <button class="sync-btn tertiary" onclick="downloadHistoryMd()">Download daily-log.md</button>
    </div>
